...
function renderGallery() {
   ...
   for (let i = currentStart; i < currentStart + imagesPerPage; i++) {
       if (images[i]) {
           galleryEl.appendChild(createGalleryItem(images[i]));
       }
   }
}
...
function prevImages() {
   ...
   if (currentStart < 0) {
       currentStart = images.length - imagesPerPage;
   }
